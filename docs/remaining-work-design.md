...
6. **repair**（最大 repairMaxIter=400 反復・1反復1修正）: demand 不足の (日,パターン) に対し順に
   ...
   atLeastGroup の不足補充は次の順に試す（②③は旧HTML版からの意図的な逸脱）:
   ① 休みの職員を充当（旧版と同じ）
   ② 2手修復: 別日の余剰勤務を休に振替→空いた職員を充当（2026-09-18 追加）。
      振替元が他の atLeastGroup を割らないか `canSpare` で確認、希望休の職員は動かさない
   ③ 同日の余剰勤務から付け替え（2026-09-24 追加。repair ②と同じ考え方）: 必要人数を超えて
      入っている勤務の職員を、不足グループの勤務へ付け替える。本人の勤務の種類が変わり
      「〜を優先」を崩しやすいため最後の手段とし、崩す数が少ない職員から試す。付け替え元も `canSpare` で守る
